<?php
// Script para testar a funcionalidade de restauração de itens da lixeira com notificação
...
";
    ...
    $stmt_mov->execute([$item_id, $novo_local_id, $lixeira_id, $novo_responsavel_id, $item_id]);
    ...
    echo "Item restaurado com sucesso e notificação criada!
";
    
    // Verificar se a notificação foi criada
    $stmt_check_notif = $pdo->prepare("SELECT id FROM notificacoes_movimentacao WHERE movimentacao_id = ? AND item_id = ? AND usuario_notificado_id = ?");
    $stmt_check_notif->execute([$movimentacao_id, $item_id, $novo_responsavel_id]);
    $notificacao = $stmt_check_notif->fetch();
    
    if ($notificacao) {
        echo "Notificação criada com sucesso! ID: " . $notificacao['id'] . "
";
    } else {
