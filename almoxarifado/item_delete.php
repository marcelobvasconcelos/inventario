...
try {
    // Primeiro, excluir as entradas relacionadas a este material
    $sql_entradas = "DELETE FROM almoxarifado_entradas WHERE material_id = ?";
    if($stmt_entradas = mysqli_prepare($link, $sql_entradas)){
        mysqli_stmt_bind_param($stmt_entradas, "i", $id);
        if(!mysqli_stmt_execute($stmt_entradas)){
    ...
    // Em seguida, excluir as saídas relacionadas a este material
    $sql_saidas = "DELETE FROM almoxarifado_saidas WHERE material_id = ?";
    if($stmt_saidas = mysqli_prepare($link, $sql_saidas)){
        mysqli_stmt_bind_param($stmt_saidas, "i", $id);
        if(!mysqli_stmt_execute($stmt_saidas)){
            throw new Exception(mysqli_error($link));
        }
    ...
    // Em seguida, excluir as movimentações relacionadas a este material
    $sql_movimentacoes = "DELETE FROM almoxarifado_movimentacoes WHERE material_id = ?";
    ...
    $sql_item = "DELETE FROM almoxarifado_materiais WHERE id = ?";
