...
if (!isset($data['item_ids']) || !is_array($data['item_ids']) || empty($data['item_ids']) || 
    !isset($data['novo_local_id']) || !isset($data['novo_responsavel_id'])) {
    echo json_encode(['success' => false, 'message' => 'Dados incompletos.']);
    exit;
}

$item_ids = array_map('intval', $data['item_ids']);
...
try {
    ...
    foreach ($item_ids as $item_id) {
        try {
            ...
            if (!$stmt_check->fetch()) {
                $erros[] = "Item {$item_id} não encontrado na lixeira.";
                continue;
            }
            ...
            $erros[] = "Erro ao restaurar item {$item_id}: " . $e->getMessage();
        }
    }
    ...
    $mensagem = "Restauração em massa concluída! {$itens_restaurados} item(s) restaurado(s) com sucesso.";
    if (!empty($erros)) {
        $mensagem .= " Erros: " . implode(', ', $erros);
    }
    
    echo json_encode([
        'success' => $itens_restaurados > 0,
        'message' => $mensagem,
    ...
    echo json_encode(['success' => false, 'message' => 'Erro ao restaurar itens: ' . $e->getMessage()]);
}
?>